...
// ===== FISSIONQUEST ORIGINAL SOFT SOUNDTRACK =====
// Original browser-generated instrumental. It does not copy any commercial melody.
let audioCtx = null;
...
const softThemeChords = [
  [196.00, 246.94, 293.66], // G - B - D
  [164.81, 196.00, 246.94], // E - G - B
  [130.81, 164.81, 196.00], // C - E - G
  [146.83, 185.00, 220.00]  // D - F# - A
];

const softThemeNotes = [
  293.66, 329.63, 392.00, 329.63,
  246.94, 293.66, 329.63, 293.66,
  261.63, 293.66, 392.00, 329.63,
  220.00, 246.94, 293.66, 246.94
];

function createSoftPad(freq, start, duration, gainValue){
  ...
  gain.gain.exponentialRampToValueAtTime(gainValue, start + 0.35);
  gain.gain.setValueAtTime(gainValue, start + duration - 0.55);
...
function createSoftPluck(freq, start, gainValue=0.055){
  const osc = audioCtx.createOscillator();
  const gain = audioCtx.createGain();
  const filter = audioCtx.createBiquadFilter();
...
function scheduleSoftTheme(){
  if(!musicOn || !audioCtx) return;

  const now = audioCtx.currentTime + 0.06;
  const chordIndex = Math.floor(musicStep / 4) % softThemeChords.length;
  const chord = softThemeChords[chordIndex];

  // Warm sustained background harmony.
  if(musicStep % 4 === 0){
    chord.forEach((freq, i)=>{
      createSoftPad(freq / 2, now, 4.2, 0.020 - i * 0.003);
      createSoftPad(freq, now, 4.2, 0.009 - i * 0.001);
    });
  }

  // Gentle original plucked line.
  createSoftPluck(softThemeNotes[musicStep % softThemeNotes.length], now, 0.050);

  // Occasional higher answer note for a calm, spacious feeling.
  if(musicStep % 4 === 2){
    createSoftPluck(softThemeNotes[(musicStep + 4) % softThemeNotes.length] * 2, now + 0.28, 0.022);
  }

  musicStep++;
}

async function startSoftMusic(button){
  if(!audioCtx){
  ...
  musicMaster.gain.exponentialRampToValueAtTime(0.72, audioCtx.currentTime + 0.8);
  musicMaster.connect(audioCtx.destination);

  musicOn = true;
  musicStep = 0;
  scheduleSoftTheme();
  musicTimer = setInterval(scheduleSoftTheme, 900);

  if(button) button.textContent = "♫ Music: On";
}

function stopSoftMusic(button){
  musicOn = false;
  if(musicTimer){
  ...
  if(musicMaster && audioCtx){
    ...
    try{
      ...
      musicMaster.gain.setValueAtTime(Math.max(musicMaster.gain.value, 0.0001), t);
      musicMaster.gain.exponentialRampToValueAtTime(0.0001, t + 0.45);
    }catch(e){}
  }
...
document.addEventListener("click", async (e)=>{
  const button = e.target.closest("#musicBtn");
  if(!button) return;

  if(musicOn){
    stopSoftMusic(button);
  }else{
    try{
      await startSoftMusic(button);
      toast("Soft Reactor Theme playing ♫");
    }catch(err){
      console.error("Audio start failed:", err);
      button.textContent = "♫ Music: Retry";
      toast("Audio could not start. Click Music again.");
    }
  }
});
